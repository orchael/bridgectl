...
**Not verified**: an actual `waiting_for_approval`/`waiting_for_input` transition from a real model turn. Two independent, externally-confirmed blockers prevented this in this sandbox, neither caused by bridgectl:

1. **The account's stored ChatGPT credential (`~/.config/bridgectl/codex-home/auth.json`) has an already-used refresh token** — `codex_login::auth::manager: Failed to refresh token: Your access token could not be refreshed because your refresh token was already used. Please log out and sign in again.` Confirmed independently of bridgectl with a plain `codex exec` against the same `CODEX_HOME`; a fresh interactive `codex login` (or a valid alternative credential) is required.
2. **A raw `OPENAI_API_KEY` fallback also failed**, but not because the key is invalid — a direct `GET https://api.openai.com/v1/models` with the same key returned `200`. Routed through bridgectl's own credential-seeding path (a fresh `CODEX_HOME` with no pre-existing `auth.json`, forcing the `resolveCodexAuth` API-key branch — see `applyCodexHomeAuth` in `internal/provider/codex.go`), the companion app-server accepted bridgectl's own observer connection's `initialize` (the connection `WatchInteraction` opens, first) but then **never responded to a second client's `initialize`** — the request just hangs, with no error on either side, reproduced twice with two different `CODEX_HOME` locations (once under `/tmp`, which the CLI separately warns about — "Refusing to create helper binaries under temporary dir" — and once outside it, ruling that out as the cause). `codex doctor` in this environment separately reports `⚠ websocket Responses WebSocket failed; HTTPS fallback may still work`, consistent with degraded outbound WSS connectivity from this sandbox to OpenAI's realtime endpoints — the most likely explanation for a second client's handshake stalling indefinitely while the first succeeded.

The `ThreadStatus`/`ThreadActiveFlag` mapping for the waiting states is grounded in the published protocol schema, not a live observation, and **should be the first thing re-verified** against a real authenticated Codex approval/input prompt — from a network environment with working outbound access to OpenAI's WebSocket endpoints, and a freshly logged-in credential — before this is relied on in production.
